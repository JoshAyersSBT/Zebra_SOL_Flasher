...
from PyQt6.QtWidgets import (
    ...
    QComboBox, QCheckBox, QSplitter, QInputDialog, QDialog
)
        ...
        try:
            if self.job.kind == "flash":
                ...
                try:
                    ...
                    for local_path, rel in project_files:
...
class ProjectEditorTab(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.base_dir = Path(__file__).resolve().parent / "projects"
    ...
    def _build(self):
        ...
        self.btn_open_project_folder = QPushButton("Open Folder")
    ...
    def on_project_changed(self, *_args):
        ...
        self.current_project_dir = self.base_dir / name if name else None
        self.current_file_path = None
        self.editor_dirty = False
        self.update_dirty_label()
        if self.current_project_dir and self.current_project_dir.exists():
            self.lbl_project_path.setText(f"Project path: {self.current_project_dir}")
            self.populate_file_list()
            main_path = self.current_project_dir / "main.py"
            if main_path.exists():
                self.load_file(main_path)
            else:
                self.clear_editor()
        else:
    ...
    def open_project_folder_dialog(self):
        path = QFileDialog.getExistingDirectory(self, "Choose projects root", str(self.base_dir))
        if not path:
            return
    ...
    def populate_file_list(self):
        ...
        for path in self.relative_files():
            rel = path.relative_to(self.current_project_dir).as_posix()
            label = f"[DIR] {rel}" if path.is_dir() else rel
            item = QListWidgetItem(label)
