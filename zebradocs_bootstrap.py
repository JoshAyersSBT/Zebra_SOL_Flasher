...
What it does
------------
- Scans the project directory to infer source roots
- Detects likely embedded / MicroPython imports and configures autodoc mocks
- Creates or updates a Sphinx docs/ tree
- Writes conf.py, index.md, api.md, architecture.md, development.md,
  requirements.txt, Makefile, make.bat
- Tries to preserve existing docs content when possible
- Can run in dry-run mode first
- When run with --force, it can also install requirements, build the docs,
  and serve them locally with Flask

Typical usage
-------------
    python zebradocs_bootstrap.py /path/to/Zebrabot
    python zebradocs_bootstrap.py . --dry-run
    python zebradocs_bootstrap.py . --force
    python zebradocs_bootstrap.py . --force --host 0.0.0.0 --port 8080
...
def detect_mock_imports(imports: Set[str]) -> List[str]:
    ...
    extras = ["machine", "uasyncio", "network", "bluetooth"]
    for mod in extras:
        if mod not in found:
            found.append(mod)
...
def make_conf_py(scan: ScanResult) -> str:
    ...
    return (
        '"""\n'
        f"Sphinx configuration for {display_name}.\n"
        ...
        f"autodoc_mock_imports = {mock_imports_repr}\n\n"
        'autoapi_type = "python"\n'
        f"autoapi_dirs = {autoapi_dirs_repr}\n"
...
def install_requirements_and_build(repo_root: Path) -> int:
    ...
    if req_file.exists():
        ...
        if code != 0:
            print("Error: failed to install documentation requirements.", file=sys.stderr)
            return code

    code = run_command(
        [python_executable(), "-m", "sphinx", "-b", "html", "docs", "docs/_build/html"],
        cwd=repo_root,
    )
    if code != 0:
        print("Error: failed to build documentation.", file=sys.stderr)
        return code

    return 0


def serve_docs_with_flask(repo_root: Path, host: str = "127.0.0.1", port: int = 8000) -> int:
    try:
        from flask import Flask, send_from_directory
    except ImportError:
        code = run_command([python_executable(), "-m", "pip", "install", "flask"], cwd=repo_root)
        if code != 0:
            print("Error: failed to install Flask.", file=sys.stderr)
            return code
        from flask import Flask, send_from_directory

    build_dir = (repo_root / "docs" / "_build" / "html").resolve()
    if not build_dir.exists():
        print(f"Error: docs build directory not found: {build_dir}", file=sys.stderr)
        return 2
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Overwrite generated docs files if they already exist, then install requirements, build docs, and serve them with Flask",
    )
    ...
    parser.add_argument("--host", default="127.0.0.1", help="Host address for the Flask docs server when using --force")
    parser.add_argument("--port", type=int, default=8000, help="Port for the Flask docs server when using --force")
...
def main(argv: List[str] | None = None) -> int:
    ...
    if args.force and not args.dry_run:
        print("\nRunning full docs pipeline because --force was provided.")
        code = install_requirements_and_build(repo_root)
        if code != 0:
            return code
        return serve_docs_with_flask(repo_root, host=args.host, port=args.port)
